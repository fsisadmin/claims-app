...
export default function AddClaimPage() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const { data: locationsData, error: locationsError } = await supabase
        .from('locations')
        .select('id, location_name, company, client_id')
        .eq('organization_id', profile.organization_id)
        .order('location_name')
  ...
  // Filter locations when client changes
  useEffect(() => {
    if (formData.client_id) {
      setFilteredLocations(locations.filter(loc => loc.client_id === formData.client_id))
    } else {
      setFilteredLocations(locations)
    }
  }, [formData.client_id, locations])

  // Handle input change
  const handleChange = (field, value) => {
    setFormData(prev => ({ ...prev, [field]: value }))

    // Reset location if client changes
    if (field === 'client_id') {
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div>
                ...
                <select
                  ...
                  className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#006B7D]/20 focus:border-[#006B7D] text-gray-900"
                  disabled={!formData.client_id && filteredLocations.length === 0}
                >
